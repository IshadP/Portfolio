...
const footerLinks = [
  {
    label: "Book a call",
    value: "cal.com/ishadpande",
    href: "https://cal.com/ishadpande",
    icon: <Icon name="calendar_today" size={20} />,
  },
  {
    label: "Email",
    value: EMAIL,
...
export default function Footer() {
  return (
    <footer className="w-full flex flex-col items-center py-12 md:py-24 border-t border-outline">
      <div className="w-full flex flex-col">
        {footerLinks.map((link, index) => (
          <a
            ...
            className="group flex justify-center w-full border-b border-outline last:border-b-0 max-md:focus:bg-bg-subtle md:hover:bg-bg-subtle transition-all duration-300 no-underline"
          >
            <div className="w-full md:w-content flex flex-row items-center justify-between px-4 py-4 md:px-0 md:py-5">
              <div className="flex items-center gap-4 text-text-primary shrink-0">
                <div className="text-text-muted group-hover:text-text-primary transition-colors duration-300">
                  {link.icon}
                </div>
                <span className="font-body-md group-hover:text-primary transition-colors duration-300">
                  {link.label}
                </span>
              </div>

              <div className="flex items-center gap-2 md:gap-4 text-text-muted group-hover:text-text-primary transition-colors duration-300 min-w-0 ml-4">
                ...
                <Icon
                  name="arrow_outward"
                  size={20}
                  className="transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300 shrink-0"
                />
              </div>
            </div>
          </a>

...
export default function SecondaryProjectCard({
  ...
  description,
  subtitle = "Youtube",
  imageLeft = "/2e9b6401fca89e7b203d8dcd6dfa64d939ba12b4.webp",
  imageRight = "/2e9b6401fca89e7b203d8dcd6dfa64d939ba12b4.webp",
  routeText = "/YOUTUBE",
  href,
  side = "left",
  className = "",
  ...
  const textPanel = (
    <div
      className={`col-span-1 self-stretch p-5 flex flex-col justify-between items-start overflow-hidden border-border-default ${
    ...
    >
      <div className="flex flex-col gap-1 w-full">
        ...
        <p className="font-body-lg-bold text-text-primary">{title}</p>
        {description && (
          <p className="font-label-md-mono text-text-muted">{description}</p>
        )}
      </div>
  ...
  return (
    <Wrapper
      ...
      transition={springTransition}
      className={`w-full max-w-5xl h-[400px] rounded-lg border border-border-default grid grid-cols-3 overflow-hidden cursor-pointer ${className}`}
    >
      {side === "left" ? (
        <>
          {textPanel}

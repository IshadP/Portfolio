...
export default function Navbar() {
    ...
    const currentPageLabel = navLinks.find((link) => link.href === pathname)?.label ?? (isProjectPage ? projectName : "Home");
                    ...
                    {isProjectPage ? (
                        ...
                        >
                            <span className="transition-transform group-hover:-translate-x-1">
                                <CaretLeftIcon size={18} weight="bold" className="text-text-secondary" />
                            </span>
                            ...
                            return (
                                <Link
                                    ...
                                    className={`self-stretch px-6 py-2 border-r border-border-default flex items-center justify-center overflow-hidden no-underline ${
                                        isActive ? "bg-active-bg" : "bg-surface-light hover:bg-bg-muted transition-colors"
                                    }`}
                                >
                                    <span className={`font-label-lg ${isActive ? "text-active-fg" : "text-text-secondary"}`}>
                                        {item.label}
                                    </span>
                                </Link>
                            );
                ...
                >
                    {isProjectPage ? (
                        ...
                        <div className="flex items-center gap-2 pointer-events-none">
                            <MenuToggleIcon isOpen={menuOpen} />
                            <span className="font-label-lg text-text-primary">{currentPageLabel}</span>
                        </div>
                    )}
                </div>
